"""
Live forward-test OPEN step for the volume-confirmed gap continuation strategy, applied to YM
(Dow futures) instead of NQ. Cloned from volume_confirmed_gap_forward_check.py after backtesting
confirmed comparable results on YM (134 trades, 42.5% win rate, PF 1.48 vs NQ's 134 trades, 43.3%
win rate, PF 1.53 at the same 1.2x threshold, same fixed stop/target).

OPEN-ONLY - resolved later in the day by volume_confirmed_gap_ym_resolve.py, matching the pattern
established for NQ after Entry 32's same-run-resolve-too-early bug.

Uses MYM (Dow micro contract, $0.50/point) sizing, not MNQ - YM's point value is different from
...
THRESHOLDS = {
    "1.2x": {
        "value": 1.2,
        "account_file": "data/volume_gap_ym_1_2x_paper_account.json",
        "risk_state_file": "data/volume_gap_ym_1_2x_risk_limits_state.json",
    },
    "1.5x": {
        "value": 1.5,
        "account_file": "data/volume_gap_ym_1_5x_paper_account.json",
        "risk_state_file": "data/volume_gap_ym_1_5x_risk_limits_state.json",
    },
}
...
print(f"[YM] Prior close: {prior_close}, Today's open: {open_price}, Gap: {gap_points:.2f} points")
print(f"[YM] Today's {ENTRY_TIME} volume: {today_entry_volume:,.0f}, trailing {VOLUME_LOOKBACK}-day avg: "
      f"{trailing_avg_volume:,.0f}, ratio: {volume_ratio:.2f}x")

if abs(gap_points) < MIN_GAP_POINTS:
    print(f"[YM] Gap too small ({gap_points:.2f} pts) - no signal for any threshold today.")
    sys.exit()
...
for label, cfg in THRESHOLDS.items():
    ...
    print(f"{label}: position opened (volume_ratio={volume_ratio:.2f}x) - "
          f"will be resolved end of day by volume_confirmed_gap_ym_resolve.py.")