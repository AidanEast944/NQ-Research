...
from strategy import get_prior_day_break_signals_atr_from_archive

# Smaller multipliers this time - scaled to session ATR (~450-480 points)
combos_to_test = [
    (0.05, 0.10),
    (0.10, 0.20),
    (0.15, 0.30),
...
print("Testing ATR-BASED DYNAMIC STOPS (smaller multipliers) against YOUR SAVED ARCHIVE (data/raw)\n")
print(f"{'ATR Stop':>9} {'ATR Target':>11} {'Trades':>7} {'Win Rate':>9} {'Avg Pts':>9} {'Total Pts':>10}")

for stop_mult, target_mult in combos_to_test:
    signals = get_prior_day_break_signals_atr_from_archive(
    ...
    log_result("prior_day_break_atr", f"atr_stop={stop_mult},atr_target={target_mult}", len(signals), win_rate, avg_points, total_points)