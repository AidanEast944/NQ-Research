...
print("Testing PRIOR DAY FADE (reversal) against YOUR SAVED ARCHIVE (data/raw)\n")
...
for stop, target in combos_to_test:
    ...
    if len(signals) == 0:
        print(f"{stop:>6} {target:>7}       0  no trades")
        continue
    ...
    log_result("prior_day_fade", f"stop={stop},target={target}", len(signals), win_rate, avg_points, total_points)